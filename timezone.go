...
func main() {
	//timeZones := Install("combined-with-oceans.json") 
	timeZones, err := LoadCompressed("test.snap")
	if err != nil {
		log.Println(err)
	}
	querys := []Coord{
...
func Install(filename string) ([]TimeZone) {
	...
	CompressAndSave(timeZones)
	return timeZones
}

func LoadCompressed(filename string) ([]TimeZone, error) {
	start_decode := time.Now()
	var tzs []TimeZone
	...
	fmt.Println("Timezones Loaded: ", len(tzs))
	//log.Println(len)

	return tzs, nil 
}

func CompressAndSave(timeZones []TimeZone)  {
